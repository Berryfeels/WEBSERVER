...
// Cleanup function
void cleanup_resources(pollfd *fds, int sockfd)
{
...
string  Request::ConnectToServer(Header head)
{
	//create a socket;
	char	request[2000];//buffer to fetch the client request text
	int		sockfd;//the fd for the server socket
	...
	//listen to request
	if (listen(sockfd, 10) == -1)//5 is the arbitrary amount accepted to que. at 6 
								//requests listen will send an error to the client
	{
	...
	while(signal_flag)
	{
		...
		else if (ready > 0)
		{
			...
			if(fds[0].revents & POLLIN)
			{
				...
				client_fd = accept(sockfd, (struct sockaddr*)&addr_client, &addr_len);
				if(client_fd == -1)
				{
					std::cout<< errno << std::endl;
					return "";
